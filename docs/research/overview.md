...
> Shared methodology, schema log, and progress tracking for the Canadian Political Data legislative-data effort. Per-jurisdiction dossiers (federal + 13 provinces/territories) are siblings of this file in `docs/research/`. See [`README.md`](./README.md) for the index. For *which* jurisdiction to research next, see [`docs/timeline.md`](../timeline.md) § Database — that's the priority order; this doc is the *how* and *what we found*.
